package com.witness.user.handler;
...
import com.witness.user.util.Utils;
 ...
 *
 */
public class BaseHandler extends PlayRTCObserver {
     ...
     * P2P연결 수립 후 PlayRTC의 P2P 상태 정보를 제공하기 위한 Report 객체
     */
    private StatsReportHandler statReportHandler = null;
    ...
    public BaseHandler(PlayRTCActivity activity) {

        this.activity = activity;

        // P2P연결 수립 후 PlayRTC의 P2P 상태 정보를 제공하기 위한 Report 객체 생성
        statReportHandler = new StatsReportHandler(activity);
    }
     ...
     */
    public void switchBackCameraFlash(LinearLayout flashBtn) {
        if(playrtc != null) {
            boolean on = !playrtc.isBackCameraFlashOn();

            if(on){
                flashBtn.setAlpha((float)0.8);
            }else{
                flashBtn.setAlpha((float)0.3);
            }
    ...
    public void onAddLocalStream(final PlayRTC obj, final PlayRTCMedia media) {
        ...
        //Locations - channelId
        activity.getLocations().setChannelId(obj.getChannelId());

        //Locations-Date
        activity.getLocations().setDate(getCurrentDate());


        activity.setLoc("TestSample");
        //firebase 사용
        final FirebaseDatabase database=FirebaseDatabase.getInstance();
        database.getReference("locations")
                .child("TestSample")
                .push().setValue(activity.getLocations())
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(final Void aVoid) {
                        Log.e("Onsuccess","Location save");
    ...
    public void sendNotification(){
        ...
        try {
            ...
            String title="Witness";
            String content="새로운 신고가 들어왔습니다.";
            ...
            jsonParam.put("to", "/topics/TestSample");
            jsonParam.put("priority", "high");
            jsonParam.put("click_action", "OPEN_ACTIVITY");
            jsonParam.put("data", notification);

            Log.i("Json File String", jsonParam.toString().replace("\\/", "/"));
            wr.write(jsonParam.toString().replace("\\/", "/"));
            wr.flush();
            wr.close();
            ...
            if (HttpResult == HttpURLConnection.HTTP_OK) {
                    Log.i(LOG_TAG,"HttpURLConnection.HTTP_OK");
            }else{
                Log.i(LOG_TAG,"No success");
            }
    ...
    public void onAddRemoteStream(final PlayRTC obj, final String peerId, final String peerUid, final PlayRTCMedia media) {
        ...
        //Log.i("onAddRemoteStream", peerId+" // "+peerUid);

        if(peerUid.contains("/")){
            String []info=peerUid.split("/");
            activity.setUid(info[1]);

            String loc=activity.getInfo().getLat()+":"+activity.getInfo().getLon();
            activity.getInfo().saveLocation(loc);
                        ...
                        .child("point")
                            ...
                            .addOnSuccessListener(new OnSuccessListener<Void>() {
                                ...
                                public void onSuccess(Void aVoid) {
                                    Log.i(LOG_TAG, "onSuccess==public");
                                    activity.setLoc("TestSample");
                                    activity.setMapClick(true);
                                }
                            })
                            .addOnFailureListener(new OnFailureListener() {
                                @Override
                                public void onFailure(@NonNull Exception e) {
                                    Log.i(LOG_TAG, "onFailure==public");
                                }
                            });
    ...
    public void onOtherDisconnectChannel(final PlayRTC obj, final String peerId, final String peerUid) {
        ...
        if(activity.getRemoteVideoView() != null) {
            ...
            activity.getVideoLayer().resizeLocalVideoView(VideoViewGroup.RTCViewSizeType.Full);
        }
    }
    ...
    */
    ...
    private PlayRTCConfig createPlayRTCConfig(int runType, String channelRing, String videoCodec, String audioCodec) {
		 ...
		 * - Back
         */
        PlayRTCVideoConfig.CameraType cameraType = PlayRTCVideoConfig.CameraType.Back;
