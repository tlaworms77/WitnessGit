...
import com.witness.gov.data.ChannelData;
...
public class ChannelView extends RelativeLayout implements ChannelListAdapter.IChannelListAdapter {
     ...
     * @see PlayRTCActivity
     * @see PlayRTCChannelViewListener
...
*/
    ...
    public void showChannelList(final NotifyItem item) {
                ...
                try {
                    ...
                    if (oData.has("error")) {
                        ...
                        for (int i = 0; i < cnt; i++) {
                            ...
                            final String channelId = channel.getString("channelId");
                            ...
                            if(item.getChannelId().equals(channelId)){
                                ...
                                Log.i("ITEM", item.getChannelId());

                                playRTC.getPeerList(channelId, new PlayRTCServiceHelperListener() {
                                            @Override
                                            public void onServiceHelperResponse(int i, String s, Object o, JSONObject jsonObject) {
                                                Log.i(LOG_TAG, "onServiceHelperResponse====");

                                                try {
                                                    JSONArray peer = jsonObject.getJSONArray("peers");
                                                    Log.i("peer", peer.toString());

                                                    listener.onClickConnectChannel(item.getChannelId(), activity.getLocation()+"/"+item.getUid(), "userName");
                                                }catch (Exception e){
                                                    e.printStackTrace();
                                                }

                                               /* listener.onClickConnectChannel(channelId, userId, userName);*/
                                            }

                                            @Override
                                            public void onServiceHelperFail(int i, String s, Object o) {
                                                Log.i(LOG_TAG, "onServiceHelperFail====");
                                            }
                                        });
